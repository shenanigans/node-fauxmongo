...
function resolveDollar (target, query, path, pathstr) {
    ...
    for (var qpath in query) {
        ...
        if (subquery instanceof Object && Object.hasOwnProperty.call (subquery, '$elemMatch')) {
            // elemMatch query
            var match = subquery.$elemMatch;
            var matchKeys = Object.keys (match);
            if (!matchKeys.length) return -1;
            if (matchKeys[0][0] == '$') {
                for (var i in pointer)
                    if (matchQuery.matchSubquery (pointer, i, match)) {
                        dollar = i;
                        break;
                    }
            } else {
                for (var i in pointer)
                    if (matchQuery (pointer[i], match)) {
                        dollar = i;
                        break;
                    }
            }
