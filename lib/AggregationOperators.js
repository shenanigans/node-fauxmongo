...
var AggregationOperators = {
    ...
    '$setEquals':       function (expression, context) {
        var sets = [];
        for (var i in expression)
            sets.push (evaluate (expression[i], context));
    ...
    '$substr':          function (expression, context) {
        ...
        var end = evaluate (expression[2], context);
    ...
    '$dayOfMonth':      function (expression, context) {
        expression = evaluate (expression[0], context);
        if (typeof expression != 'date')
